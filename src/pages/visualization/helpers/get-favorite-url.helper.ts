export function getFavoriteUrl(favorite: any): string {
  ...
  let url: string = favorite.type + 's/' + favorite.id + '.json?fields=';
  if (favorite.type === 'map') {
    url += `id,user,displayName,longitude,latitude,zoom,basemap,mapViews[*,organisationUnitGroupSet[id,name,displayName,
    ...
    url += `*,interpretations[id,type,text,created,likes,likedBy[id,name],user[id,name],comments[id,created,text,user[id,name]],
