...
export class InterpretationService {
  ...
  create(interpretationData: any, rootUrl: any) {
    return new Observable(observer => {
      ...
      this.httpClient.post(url, interpretationData.message).subscribe(() => {
        const interpretationsUrl = rootUrl + interpretationData.type + 's/' + interpretationData.id +
        ...
        this.httpClient.get(interpretationsUrl)
  ...
  edit(interpretation: any, rootUrl) {
    return new Observable(observer => {
      this.httpClient.put(rootUrl + '/interpretations/' + interpretation.id, interpretation.text)
        .subscribe(() => {
  ...
  delete(interpretation: any, rootUrl) {
    return this.httpClient.delete(rootUrl + '/interpretations/' + interpretation.id);
  }

  updateLikeStatus(interpretation: any, rootUrl: string, like: boolean = true) {
    return new Observable(observer => {
      const likePromise = like ? this.httpClient.post(rootUrl + 'interpretations/' + interpretation.id + '/like', {}) :
        this.httpClient.delete(rootUrl + 'interpretations/' + interpretation.id + '/like');
  ...
  postComment(interpretation: any, rootUrl: string) {
    return new Observable(observer => {
      this.httpClient.post(rootUrl + 'interpretations/' + interpretation.id + '/comments', interpretation.comment)
        .subscribe(() => {
          this.getInterpretation(interpretation, rootUrl)
            .subscribe((interpretation) => {
              observer.next(interpretation);
  ...
  editComment(rootUrl: string, interpretation: any, comment: any) {
    return new Observable(observer => {
      this.httpClient.put(rootUrl + 'interpretations/' + interpretation.id + '/comments/' + comment.id, comment.text)
        .subscribe(() => {
          this.getInterpretation(interpretation, rootUrl)
            .subscribe((interpretation) => {
              observer.next(interpretation);
